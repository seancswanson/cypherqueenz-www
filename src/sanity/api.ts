...
export async function getAllBlogPosts(): Promise<BlogPost[]> {
    const client = useSanityClient();
    const query = "*[_type == 'post' && visibleBoolean == true]{title, author, excerpt, slug, _createdAt, slug, content[], galleryImages-> }";
    const media = await client.fetch(query);
    return media.sort((a: any, b: any) => {
        return a._createdAt > b._createdAt ? 1 : -1;
    });
...
export async function getAllEventPosts(): Promise<EventPost[]> {
    const client = useSanityClient();
    const query = "*[_type == 'event' && visibleBoolean == true]{title, slug, _createdAt, slug, time, date, location, entryFee, content[], blogAndEventImages-> }";
    const media = await client.fetch(query);
    return media.sort((a: any, b: any) => {
        return a._createdAt > b._createdAt ? 1 : -1;
    });
